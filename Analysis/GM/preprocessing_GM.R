...
test <- Supplementary_Data_2_08272026 %>% filter(!is.na(SGE))%>% dplyr::select(`#CHROM`, POS_hg38,REF, ALT,  AA,
...
# Recreate EventType 
out_final <- test %>% mutate(uPOS = paste(POS, REF, ALT, sep = "_")) %>% filter(Exon != "E10") %>%
  mutate(
    EventType = case_when(
